...
if [ -z "$version" ]; then
  version="$(wget -q -O- "https://api.github.com/repos/StellarWitch7/element-desktop/releases?per_page=1" | jq -r '.[0].tag_name')"
fi
...
# Element Web
web_src="https://raw.githubusercontent.com/StellarWitch7/element-web/v$version"
web_src_hash=$(nix-prefetch-github StellarWitch7 element-web --rev v${version} | jq -r .hash)
...
# Element Desktop
desktop_src="https://raw.githubusercontent.com/StellarWitch7/element-desktop/v$version"
desktop_src_hash=$(nix-prefetch-github StellarWitch7 element-desktop --rev v${version} | jq -r .hash)
